...
> 一个面向**零基础小白**的 Spring AI 学习项目。把 [Spring AI 官方文档](https://docs.spring.io/spring-ai/reference/index.html) 的每个核心知识点拆成一个**独立可运行的 Maven 模块**，按 `01 → 20` 的顺序循序渐进。每个模块都有**详细中文注释 + 流程图 + 独立 README**。
...
本项目把官方文档左侧菜单的核心知识点，整理成 20 个学习模块：
...
| 17 | `17-prompt-engineering` | 提示工程模式 | 零样本/少样本/角色/思维链/参数控制 |
| 18 | `18-agents` | 构建智能体 | 提示链/路由/并行/评估-优化 等编排模式 |
| 19 | `19-mcp-client` | MCP 客户端 | 连接并调用 14 暴露的 MCP 工具（成闭环） |
| 20 | `20-moderation` | 内容审核 | 检测违规内容（需 OpenAI） |

> **建议学习顺序**：严格按 01 → 20。前 8 个是对话核心，必学；09~11 是 RAG 知识库三件套；12~16 是进阶专题；17~20 是扩展专题（提示工程/智能体/MCP 闭环/内容审核）。
